...
# note to self:
# FullVisibilityXAttention is for sending kv info from encoder to the 
# queries of the decoder
class FullVisibilityXAttention(nn.Module):
    """
    FullVisibility means that the query has access to all the keys and values,
...
class FullVisibilityXBlock(nn.Module):
    def __init__(self, embedding_dim, num_heads, factor):
        super().__init__()
        self.embedding_dim = embedding_dim
        self.num_heads = num_heads
        self.factor = factor
        self.attn = FullVisibilityXAttention(embedding_dim, num_heads, factor)
...
# note to self:
# OneVisibilityXAttention is for sending kv info from the decoder to the
# queries of the encoder
class OneVisibilityXAttention(nn.Module):
    """
    OneVisibility means that the query has access to one key and value,
    specifically the first key and value
    """
    def __init__(self, embedding_dim, num_heads, factor):
        super().__init__()
        self.embedding_dim = embedding_dim
    ...
    def forward(self, q_x, kv_x):
        # first select the first key and value
        kv_x = kv_x[:,0:1,:]
        batch_size, seq_len, embedding_dim = q_x.shape
        q = self.q_attn_proj(q_x).view(batch_size, -1, self.num_heads, embedding_dim//self.num_heads).transpose(1,2)
        k, v = self.kv_attn_proj(kv_x).split(embedding_dim, dim=-1)
...
class OneVisibilityXBlock(nn.Module):
    def __init__(self, embedding_dim, num_heads, factor):
        super().__init__()
        self.embedding_dim = embedding_dim
        self.num_heads = num_heads
        self.factor = factor
        self.attn = OneVisibilityXAttention(embedding_dim, num_heads, factor)
        self.ffn = phm(factor, embedding_dim, embedding_dim*4)
        self.ffn2 = phm(factor, embedding_dim*4, embedding_dim)
        self.norm1 = nn.LayerNorm(embedding_dim)
        self.norm2 = nn.LayerNorm(embedding_dim)
    ...
    def forward(self, x, kv_x):
        attn = self.attn(self.norm1(x), self.norm1(kv_x))
        x = x + attn
        ffn = self.ffn_activation(self.ffn(self.norm2(x)))
        ffn = self.ffn2(ffn)
        x = x + ffn
